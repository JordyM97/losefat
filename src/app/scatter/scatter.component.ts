...
async getSales(){
  await this.loginservice.getvisita().subscribe(
    result => {
      for(let clave in result){
        ...
        console.log(this.productos.map(d =>{
          console.log(d.Contador.toString())
        }));
...
private createColors(Data:any): void {
  ...
  .domain(Data.map(d => d.Contador.toString()))
  .range(["#c7d3ec", "#a5b8db", "#879cc4", "#677795", "#5a6782"]);
}
private drawChart(Data:any): void {
  // Compute the position of each group on the pie:
  const pie = d3.pie<any>().value((d: any) => Number(d.Contador));
